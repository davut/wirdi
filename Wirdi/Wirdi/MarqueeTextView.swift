...
struct SpeechScrollView: View {
    ...
    @State private var pendingCenterWordIndex: Int? = nil
        ...
        GeometryReader { geo in
            WordFlowLayout(
                ...
                onWordTap: { charOffset in
                    // Block automatic re-centering so it doesn't fight this animation
                    justTapped = true
                    manualOffset = 0
                    let index = wordIndex(forCharOffset: charOffset)
                    withAnimation(.spring(response: 0.25, dampingFraction: 0.85)) {
                        centerOnWord(index, containerHeight: containerHeight)
                    }
                    onWordTap?(charOffset)
                    // Allow automatic centering again after animation completes
            ...
            .onPreferenceChange(WordYPreferenceKey.self) { positions in
                ...
                if let pending = pendingCenterWordIndex {
                    centerOnWord(pending, containerHeight: containerHeight)
                }
            }
            ...
            .onChange(of: highlightedCharCount) { _, _ in
                if isListening && !smoothScroll && !justTapped {
                    manualOffset = 0
            ...
            .overlay(
                ScrollWheelView(
                    onScroll: { delta in
                        ...
                        let upperBound = containerHeight * scrollAnchor
                        let lowerBound = -max(0, maxY - containerHeight * scrollAnchor)
    ...
    /// Anchor ratio: 0.0 = top, 0.5 = center, 1.0 = bottom.
    /// Using 0.35 places the active word in the upper third so more upcoming text is visible below.
    private let scrollAnchor: CGFloat = 0.35

    private func recalcCenter(containerHeight: CGFloat) {
        let anchor = containerHeight * scrollAnchor
        ...
        if smoothScroll {
            ...
            centerOnWord(wordIdx, containerHeight: containerHeight)
        }
    }

    private func wordIndex(forCharOffset charOffset: Int) -> Int {
        var offset = 0
    ...
    private func centerOnWord(_ index: Int, containerHeight: CGFloat) {
        guard containerHeight > 0 else {
            pendingCenterWordIndex = index
            return
        }
        guard let wordY = wordYPositions[index] else {
            pendingCenterWordIndex = index
            return
        }
        let anchor = containerHeight * scrollAnchor
    ...
    private func wordProgressAtCurrentOffset() -> Double {
        let anchor = containerHeight * scrollAnchor
        // The Y position currently at the anchor point of the view
...
struct WordFlowLayout: View {
    ...
    var onWordTap: ((Int) -> Void)? = nil
    @State private var tappedWordId: Int? = nil
    @State private var cachedItems: [WordItem] = []
    @State private var cachedLines: [[WordItem]] = []

    private func rebuildCache() {
        cachedItems = buildItems()
        cachedLines = buildLines(items: cachedItems)
    }
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 2) {
            ForEach(Array(cachedLines.enumerated()), id: \.offset) { _, line in
                HStack(spacing: 0) {
        ...
        .onChange(of: words) { _, _ in rebuildCache() }
        .onChange(of: containerWidth) { _, _ in rebuildCache() }
    }
    ...
    private func wordView(for item: WordItem, isNextWord: Bool) -> some View {
        ...
        let isTapped = tappedWordId == item.id
        ...
        if !highlightWords {
            ...
            return Text(item.word + " ")
                ...
                .foregroundStyle(isTapped ? .white : uniformColor)
                .padding(.vertical, 4)
                .background(
        ...
        if item.isAnnotation {
            ...
            return Text(item.word + " ")
                ...
                .foregroundStyle(isTapped ? .white : annotationColor)
                .padding(.vertical, 4)
                .background(
                    GeometryReader { wordGeo in
                        Color.clear.preference(
        ...
        return Text(item.word + " ")
            .font(Font(font))
            .foregroundStyle(isTapped ? .white : wordColor)
    ...
    private func tapWord(_ item: WordItem) {
        // Brief flash to confirm the tap
        withAnimation(.easeOut(duration: 0.1)) {
            tappedWordId = item.id
        }
